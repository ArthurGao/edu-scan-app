"""
Generate additional K12 seed problems using Gemini (free tier) and export as migration 016.

Features:
  - Concurrent topic processing (asyncio semaphore)
  - Incremental progress saved to .progress_016.jsonl after each problem
...
Gemini free tier (Google AI Studio):
  - gemini-2.5-flash-lite: 1500 requests/day, 30 RPM — default concurrency=3
...
Usage (from backend/ directory):
    ...
    python -m scripts.generate_seed_problems_gemini --export-migration --concurrency 5
    python -m scripts.generate_seed_problems_gemini --clear-cache   # discard progress and restart
"""
...
PROGRESS_FILE = Path(__file__).parent / ".progress_016.jsonl"

# ──────────────────────────────────────────────
# Topics — same coverage as 015 but different problems
# ──────────────────────────────────────────────
...
def _fix_latex_escapes(text: str) -> str:
    ...
    # Gemini sometimes returns \frac, \sqrt etc. as single backslashes,
    ...
    for rec in SEED_RECORDS:
        ...
        conn.execute(
            sa.text("""
                ...
                VALUES
                    (:scan_id, \'gemini\', \'seed-data\', :content, :steps::jsonb,
...
def write_migration(records: list[dict], output_dir: Path) -> Path:
    ...
    # JSON uses null/true/false; convert to Python equivalents for the migration file
    records_json = records_json.replace(": null", ": None").replace(": true", ": True").replace(": false", ": False")
    ...
    out_path = output_dir / "016_seed_problems_gemini.py"
...
async def main(
    ...
    log.info("Next steps:")
    log.info("  git add alembic/versions/016_seed_problems_gemini.py && git commit")
    log.info("  Deploy command: alembic upgrade head && python -m scripts.backfill_embeddings")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Generate K12 seed problems via Gemini (free) → migration 016")
    parser.add_argument("--export-migration", action="store_const", const="export-migration", dest="mode",
                        help="Write alembic/versions/016_seed_problems_gemini.py (default)")
    ...
    parser.add_argument("--concurrency", type=int, default=3,
                        help="Max concurrent LLM calls (default: 3, Gemini 30 RPM limit)")
